...
Information for both Oysters and Red Abalone temperature and depth ranges come from SeaLifeBase[^4].
...
To determine the average seat surface temperature across the west coast an average from 2008-2012 data was calculated. Rasters were created by NOAA using satellite data from Coral Reef Watch[^5].
...
The West Coast Exclusive Economic Zone data represents designated coastal areas designating state control over the natural resources. These maritime boundaries come from marineregions.org[^7].
...
```{r, include=FALSE}
sst_08 <- rast(here("posts", "aquaculture-final-project", "data", "average_annual_sst_2008.tif"))

sst_09 <- rast(here("posts", "aquaculture-final-project", "data", "average_annual_sst_2009.tif"))

sst_10 <- rast(here("posts", "aquaculture-final-project", "data", "average_annual_sst_2010.tif"))

sst_11 <- rast(here("posts", "aquaculture-final-project", "data", "average_annual_sst_2011.tif"))

sst_12 <- rast(here("posts", "aquaculture-final-project", "data", "average_annual_sst_2012.tif"))

depth <- rast(here("posts", "aquaculture-final-project", "data", "depth.tif"))

eez <- st_read(here("posts", "aquaculture-final-project", "data", "wc_regions_clean.shp"))
...
To determine the suitable habitat, the reclassified temperature and depth rasters of 0’s and 1’s are multiplied together, resulting in 1’s only in cells that had a suitable depth and temperature range and 0’s in all other cells.
...
```{r, include=FALSE}
# Check new raster to make sure multipilcation worked
ocean[[1]]
names(ocean) <- 'cells'
```

# Map suitable habitat from multiplied rasters
...
suitable_cell_area <- suitable_cells %>%
  mutate(suitable_habitat = cells * cell_area$area)

# Table with calculated cells and area per region
suitable_cell_area %>% 
  arrange(desc(cells)) %>% 
  kable(col.names = c("Region", "Number of cells with suitable habitat", "Area of suitable habitat (sq km)"), caption = "Suitable habitat by EEZ region") %>%
  kable_styling(bootstrap_options = "striped", full_width = FALSE)
```
