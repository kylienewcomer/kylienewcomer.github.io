...
```
## Using a hurdle model without upwelling predictor variable
Because of the hurdle model structure it is hard to visualize how confounding variables influence the response. I removed upwelling from the model and bootstrapped to determine how upwelling impacts the relationship between SST and YOY abundance. 
...
ggplot(boot2, aes(SST_D30, count)) +
  ...
  theme_light()
```
Without upwelling, temperature appears to have less of an effect on YOY abundance. Additionally, the CI at both tail ends are noticebly larger than in the model including upwelling. While no definititve conclusions can be made from the data about impacts of SST and upwelling, there does appear to be a negative relationship between temperature and YOY abundance, with that relationship increasing with the addition of upwelling.
